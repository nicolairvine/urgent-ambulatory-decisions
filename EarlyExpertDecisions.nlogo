;;extensions[time csv]            ;; for use with time extension (not available for online model function as of 2025)
globals [
;;time-related
;;  dt                            ;; for logotime and anchoring use with time extension (not available for online model function as of 2025 but supported in desktop versions)
...
;  slow-arrivals
  ...
  random-adm-aec                  ;; comparator for aec patients converted to admission upon final disposition review
  random-discharge-amu            ;; comparator of in-patient patients converted to discharge upon final disposition review
  expert-adj-sn                   ;; max expert-adjust value of nursing staff for sampling distribution
  expert-adj-trainee              ;; mean expert-adjust value of trainee for sampling distribution
...
to setup
  ca
;;  set tick-datetime time:anchor-to-ticks (time:create "2019-09-30 00:00") 0.01666667 "hour"  ;; for use with time extension (not available for online model function as of 2025)
  ...
  set amu-crowding overcrowding-tolerance
  set max-ip-waits 3
  ...
  set expert-adj-sn 0.1
  set expert-adj-trainee 0
...
;   export-plot "Occupancy" (word "C:/....".csv")                    ;; to collect occupancy data assign folder - note this will be very large
;  ]
   stop]
;;  output-print time:show tick-datetime "EEEE dd-MM-yyy HH:mm"      ;; for use with time extension (not available for online model function as of 2025)
...
;; Environment hard coded as no plan to adjust resources beyonf staffing for project work but could be altered to a slider to adjust beds/clinic space
...
;PATIENTS STILL TO COME IN
  ask patients with [label = "QAMU" or label = "QAEC"] [                              ;; queuing patients will check to see if arrival time has been met
    if time_arrived = ticks
     [relocate
   ;  set arrival-times lput (list ed time:show tick-datetime "EEEE dd-MM-yyy HH:mm") arrival-times               ;; will relocate & record arrival time if validation require
     ]
        if time_arrived < ticks
     [relocate
  ;   set arrival-times lput (list ed time:show tick-datetime "EEEE dd-MM-yyy HH:mm") arrival-times               ;; safety net to ensure all suitable patients have moved during verification
     ]]
...
to dispose                                                                           ;; procedure to identify where patients should move to once care complete
 ...
 ifelse for-discharge = true
        [set shape "home-car"                                                        ;; identify for home at GUI
         set label "H"
...
;    set time_in_system([time_in_system] of self * minutes-per-tick)                 ;; time spent in urgent care system (from time of referral) in mins
 ...
 ;   (list label time:show tick-datetime "EEEE dd-MM-yyy HH:mm") departure-times     ;; departure-times collected for validation as needed
    die
  ]
...
;    set time_in_system([time_in_system] of self * minutes-per-tick)                 ;; time spent in urgent care system (from time of referral) in mins
    set time-variables lput
    (list label los delayed hrqol) time-variables                                    ;; departure-times collected for validation as needed
;    set departure-times lput
...
if label = "H" and AEC-ok? = true[
    ...
    set los ([los] of self * minutes-per-tick)                                       ;; los in amu area as mins
    set delayed ([delayed] of self * minutes-per-tick)
;    set time_in_system([time_in_system] of self * minutes-per-tick)                 ;; time spent in urgent care system (from time of referral) in mins for validation
    set time-variables lput
    (list label los delayed hrqol) time-variables                                    ;; separate csv file for analysis as needed
;    set departure-times lput
;    (list label time:show tick-datetime "EEEE dd-MM-yyy HH:mm") departure-times     ;; departure-times collected for validation as needed
...
to collect-patient-data    ;;example of data collection to validate patient flow
...
to-report admissions-daily

  let gui-complete (amu-complete + aec-complete)
  ifelse gui-complete = 0 [set gui-complete 0.0000000000000001][set gui-complete gui-complete]
  let gui-admission ((amu-complete + aec-complete) - (AMU-disch + AEC-disch))
;;    while [gui-admission = 0] [set gui-admission 0.0000000000000001]

  let display-admissions ticks
  ifelse display-admissions <= week * 2
    [report 0] [report (gui-admission / gui-complete) * 100]
...
"" ""
PENS
"Bedded area" 1.0 0 -13345367 true "" "plot occupancy-AMU"
"AAA" 1.0 0 -7171555 true "" "plot occupancy-AEC"
"Departmental occupancy" 1.0 0 -16777216 true "" "plot occupancy-total"
"100% Threshold" 1.0 0 -955883 true "" "plot 100 "
...
1082
713
Transferred (admitted) after AEC closure
capacity-moves-daily
17
1
11
...
1382
339
Note: Daily Waits & Admissions show invalid data until 2-week warm-up period complete
11
14.0
1
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot admissions-daily"
...
1340
617
Tally of persons diverted due to zero capacity in the system
11
0.0
1
...
1312
660
Tally of patients forced to commence care in non-allocated place due to limited capacity
11
0.0
1
...
1337
701
Patients transferred out-of-hours (note out-of-hours indicated by AEC closure time)
11
0.0
1
...
1291
756
Tally of referrals advised not to attend
11
0.0
1
...
926
597
Pool of staff able to rotate through shifts
11
0.0
1
...
1334
803
Referrals awaiting allocation (verification output)
11
0.0
1
...
748
628
REMEMBER TO SET PARAMETERS BEFORE STARTING THE MODEL
11
14.0
1

@#$#@#$#@
This is an online friendly version of the Early Expert Decision-Maker model
Created by Dr Nicola J Irvine

Liscensed for use under Creative Commons Attribution CC-BY-4.0

Recipents redistributing the work must give credit to the original author of the work (= attribution) and state changes if any, including a URL or link to the original work, this CC-BY licence and a copyright notice. Author can request to remove any attribution given information. 

Recipients re-distributing the work to third parties may not apply legal terms or technological measures (like Tivoisation) that legally restrict the rights granted by the licence. OKF (Open Knowledge Fundation) recommends this licence. The European Commission has adopted CC-BY-4.0 for sharing documents.


Time extenstion removed as not compatible with online model functioning but original coding left in the document
1 model time-step == 1 minute

Model runs for 18weeks with a 2-week warm up period (limited outputs displayed)
Recommend running at the highest speed


## WHAT IS IT?
This model explore how the expertise of clinical staff making early, remote decisions about out-patient versus in-patient care effects departmental efficiency in an acute medical unit.

Acute medical unit are highly dynamic areas of urgent care specialising in internal medical emergencies. They may receive patients directly from the community or an emergency department. All patients must be referred to a member of AMU staff to decide if they are unlikely to need overnight admission to hospital.

If unlikely for admission, they are allocated for evaluatino in the out-patient facilities (ambulatory assessment area). If likely, the in-patient are (bedded). As this decision requires clinical and hospital system expertise to know what may safely and effectively be managed without admission, senior clinicians (consultants/attendings) have the highest degree of expertise in identifying suitability for avoiding admission.

As this is not common practice (due to costs of staffing resources) and has limited evidence to demonstrate the value of experts over non-experts, the model seek to understand how the dynamics of the department change as the decision-makers do.

## HOW IT WORKS
Patients are referred to the unit from ED or non-ED (community) sources with a concern about an internal medical emergency. The unit staff taking the call decide if the patient's needs suggest admission avoidance is suitable and allocate accrodingly. Experts (senior doctors) are also able to detect patients in whom no emergency exists and are safe to avoid attending altogether (non-attenders).

Patients then travel to hospital and seek clinical resources in their allocated area so they can start evaluation & treatment straight away. If none available they will wait, but at some point (1hr bedded; 4hrs AAA) they will seek a clinical resource in any area in direct competition with other patients. Bedded area allocated patients will always seek to move into a bed when one becomes available even if started evalaution in the AAA due to limited capacity

Once their care is complete, they are evaluated for admission or discharge and exit the model. 


### Emergent outcomes with influence upon patient flow
Model exit can be delayed if they are identified for discharge overnight due to transport availability. Exit may also be delayed in admitted patients if: 
1) demand upon the unit in the last 24hrs has been excessive (>65 referrals -> very high hospital occupancy)
2) there are lots of bedded area resources in the unit as the hospital is assumed to be in high occupancy on all days
3) overnight to limit disruption to patient rest

Model exit delays for admitted patients can be overuled and transfer expedited if:
- the bedded area exceeds the threshold for safe occupancy tolerance 
- the anticipated number of patients arriving to the bedded area exceeds the bed available


## HOW TO USE IT
All demand parameter default to realistic UK urban setting prevelances

1. Set the mean daily demand and start/end times of peak referral activity

2. Choose the opening and closing times of the AAA facilties (labelled AEC)
The prevalence of AAA suitability in  ED and non-ED popualtions can also be adjusted to reflect local values

3. Chose how much overcrowding (bedded area occupancy) will be tolerated before patients are admitted before care is complete

4. Select the staff to allocate across all hours and days
    - Full-time staff will take all referrals if not other staff groups are present
    - Nursing staff will take only ED referrals when trainee or expert clinicians are on         shift
    - Trainees will take only ED referrals is on shift with an expert and only non-ED            referrals when on with nursing staff
    - Experts will take only non-ED referrals if working with another staff group

5. Click set parameters to ensure any changes are set otherwise the model will run with the default values


The outputs provide the key measures of efficiency collected in the model:
- admissions indicating system efficiency (% of all patients who were transfered into the   hospital after care completion)
- delays per day indicating departmental efficiency
- overcrowding indicating departmentla efficiency
- system failure events (patients diverted because of zero capacity)



### Additional expert decision logic (not active)
Experts may also increase the number of patients they allocate to the AAA if they sense overcrowding (>100% occupancy). This is actual overcrowding or anticipated overcrowding when numbers referred exceed bed available. This is hard programmed to zero in the code because model validation revealed no meaningful difference in outcomes when activated with realistic values.
